...
@Component({
  selector: 'app-campoharmonico-f',
  templateUrl: './campoharmonico-f.component.html',
  styleUrls: ['./campoharmonico-f.component.css']
})
export class CampoharmonicoFComponent {
  ...
  tela = 'exe1';
    ...
    resultado() {
    if (this.qtd_erros == 0){
        this.estrelasCF = this.estrelasCF + 3
    }
    if (this.qtd_erros <= 3 && this.qtd_erros > 0){
      this.estrelasCF = this.estrelasCF + 2
    }
    if (this.qtd_erros > 3){
      this.estrelasCF = this.estrelasCF + 1
    }
      this.tela = 'resultado';
      console.log('QTD_ESTRELAS',this.estrelasCF);
      }
