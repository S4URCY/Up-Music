...
@Component({
  selector: 'app-notas-menores-d',
  templateUrl: './notas-menores-d.component.html',
  styleUrls: ['./notas-menores-d.component.css'],
  encapsulation: ViewEncapsulation.None
})
export class NotasMenoresDComponent {
  ...
  resultado() {
  if (this.qtd_erros == 0){
      this.estrelasMenoresD = this.estrelasMenoresD + 3
  }
  if (this.qtd_erros <= 3 && this.qtd_erros > 0){
    this.estrelasMenoresD = this.estrelasMenoresD + 2
  }
  if (this.qtd_erros > 3){
    this.estrelasMenoresD = this.estrelasMenoresD + 1
  }
    this.tela = 'resultado';
    console.log('QTD_ESTRELAS',this.estrelasMenoresD);
    }
