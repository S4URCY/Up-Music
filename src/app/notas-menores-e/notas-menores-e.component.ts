...
@Component({
  selector: 'app-notas-menores-e',
  templateUrl: './notas-menores-e.component.html',
  styleUrls: ['./notas-menores-e.component.css']
})
export class NotasMenoresEComponent {
  ...
  resultado() {
  if (this.qtd_erros == 0){
      this.estrelasMenoresE = this.estrelasMenoresE + 3
  }
  if (this.qtd_erros <= 3 && this.qtd_erros > 0){
    this.estrelasMenoresE = this.estrelasMenoresE + 2
  }
  if (this.qtd_erros > 3){
    this.estrelasMenoresE = this.estrelasMenoresE + 1
  }
    this.tela = 'resultado';
    console.log('QTD_ESTRELAS',this.estrelasMenoresE);
    }
