...
@Component({
  selector: 'app-campoharmonico-d',
  templateUrl: './campoharmonico-d.component.html',
  styleUrls: ['./campoharmonico-d.component.css'],
  encapsulation: ViewEncapsulation.None

})
export class CampoharmonicoDComponent {
  ...
  resultado() {
  if (this.qtd_erros == 0){
      this.estrelasCD = this.estrelasCD + 3
  }
  if (this.qtd_erros <= 3 && this.qtd_erros > 0){
    this.estrelasCD = this.estrelasCD + 2
  }
  if (this.qtd_erros > 3){
    this.estrelasCD = this.estrelasCD + 1
  }
    this.tela = 'resultado';
    console.log('QTD_ESTRELAS',this.estrelasCD);
    }
