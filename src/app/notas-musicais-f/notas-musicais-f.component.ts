...
@Component({
  selector: 'app-notas-musicais-f',
  templateUrl: './notas-musicais-f.component.html',
  styleUrls: ['./notas-musicais-f.component.css']
  
})
export class NotasMusicaisFComponent {
  ...
  resultado() {
  if (this.qtd_erros == 0){
      this.estrelasF = this.estrelasF + 3
  }
  if (this.qtd_erros <= 3 && this.qtd_erros > 0){
    this.estrelasF = this.estrelasF + 2
  }
  if (this.qtd_erros > 3){
    this.estrelasF = this.estrelasF + 1
  }
    this.tela = 'resultado';
    console.log('QTD_ESTRELAS',this.estrelasF);
    }
